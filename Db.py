...
def createDb(userList):
	...
	try:
		...
		c.execute('''CREATE TABLE IF NOT EXISTS Profiles (id integer PRIMARY KEY, username text, fullname text, position text, description text, location text, imgURL text, encoding text, encryption text, decryptionKey text, status text, secretKey text)''')
...
def saveProfile(dictionary, username):
    ...
    data = c.fetchone()
    if data == None:
        c.execute('''INSERT INTO Profiles (username, fullname, position, description, location, imgURL) VALUES (?,?,?,?,?,?)''', (username, dictionary.get('fullname'), dictionary.get('position'), dictionary.get('description'), dictionary.get('location'), dictionary.get('imgURL')))
    else:
        c.execute('''UPDATE Profiles SET username=?, fullname=?, position=?, description=?, location=?, imgURL=? WHERE rowid=?''', [username, dictionary.get('fullname'), dictionary.get('position'), dictionary.get('description'), dictionary.get('location'), dictionary.get('imgURL'), data[0]])
    c.close()
    conn.commit()
    conn.close()
    return True
...
def getProfile(username):
    ...
    userProfileData = [dict(zip(['rowid', 'username', 'fullname', 'position', 'description', 'location', 'imgURL', 'encoding', 'encryption', 'decryptionKey', 'status', 'secretKey'], row)) for row in c.fetchall()]
    c.close()
    conn.commit()
    conn.close()
